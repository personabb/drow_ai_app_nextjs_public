...
class SDXLC:
    def __init__(self,device = None, config_ini_path = './configs/config.ini'):
        ...
        self.base, self.refiner = self.preprepare_model(mode = "sketch")


    def preprepare_model(self, controlnet_path = None, mode = "sketch"):
        ...
        if mode == "sketch":
            ...
            if self.VAE_FLAG:
                ...
                if not self.SINGLE_FILE_FLAG:
                    ...
                    base = StableDiffusionXLControlNetPipeline(controlnet = controlnet_s, **pipe.components)
            ...
            if self.VAE_FLAG:
                ...
                if not self.SINGLE_FILE_FLAG:
                    ...
                    base = StableDiffusionXLInpaintPipeline(**pipe.components)
            ...
            raise ValueError("mode is only 'sketch' or 'inpaint'.")


        if self.LORA_FLAG:
            base.load_lora_weights(self.lora_weight_path, adapter_name="lora1")
            if self.LORA_FLAG2:
                base.load_lora_weights(self.lora_weight_path2, adapter_name="lora2")
                base.set_adapters(["lora1", "lora2"], adapter_weights=[1.0, 1.0])
    ...
    def memory_reset_model(self, mode = "sketch"):
        ...
        torch.cuda.empty_cache()
        GPUtil.showUtilization()
        self.base , self.refiner = self.preprepare_model(mode = mode)
